...
def replace_generated_block(content: str, block: str, anchor: str) -> str:
    ...
    ranges = _managed_block_ranges(content)
    if ranges:
        start_index, end_index = ranges[0]
        updated = f"{content[:start_index]}{block}{content[end_index:]}"
        duplicate_ranges = _managed_block_ranges(updated)[1:]
        for duplicate_start, duplicate_end in reversed(duplicate_ranges):
            updated = f"{updated[:duplicate_start]}{updated[duplicate_end:]}"
        return updated
...
def _managed_block_ranges(content: str) -> list[tuple[int, int]]:
    ...
    cursor = 0
    while True:
        start_index = content.find(START_MARKER, cursor)
        if start_index == -1:
            return ranges
        ...
        if content[end_index : end_index + 1] == "\n":
            end_index += 1
        if not _inside_fenced_code(content, start_index):
